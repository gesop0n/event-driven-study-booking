...
![SeatReleased への合流](diagrams/seat-released-merge.svg)

**`ReservationConfirmed` に2つの経路がある。** 申込から直接届く道と、
補欠の繰り上げから届く道。後者は購読者（補欠繰り上げ）が生んだイベントなので、
ここで流れが一巡して戻ってくる。

![ReservationConfirmed に届く2つの経路](diagrams/reservation-confirmed-paths.svg)


### 設計上の判断

**`SeatReleased` は原因を問わない統一イベントとする。**
席が空く原因は「キャンセル」や「定員の増加」と複数あるが、繰り上げ処理が知りたいのは
「Aさんがキャンセルした」ではなく「席が1つ空いた」という事実なので、抽象度を保つ。
これにより、原因が増えても繰り上げ処理を変更せずに済む。
...
- `RegistrationClosed`（募集が締め切られた）— **購読者がいないため**
  - 募集終了は申込時に現在時刻と募集締切を比べれば判定でき、一覧画面の「募集終了」表示も同じ比較で導ける。誰もイベントを必要としていないため
